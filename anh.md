# Tiếng Anh – Unit 8, 9, 10

> File này dùng cho `anh.md` trong web học bài.  
> Nội dung: **Environment**, **Travel & Tourism**, **Technology & Education**, kèm cụm quan trọng cần học thuộc.

---

# UNIT 8 – ENVIRONMENT

## 1. Từ vựng pronunciation + options

| Từ / Cụm từ | Nghĩa |
|---|---|
| ban (v/n) | cấm / lệnh cấm |
| damage (n,v) | thiệt hại; làm hư hại |
| landfill (n) | bãi chôn rác |
| material (n) | vật liệu |
| recycle (v) | tái chế |
| compost (n,v) | phân hữu cơ; ủ phân |
| cattle (n) | gia súc, bò |
| community (n) | cộng đồng |
| release (v,n) | thải ra, giải phóng |
| livestock (n) | vật nuôi |
| mixture (n) | hỗn hợp |
| packaging (n) | bao bì |
| obvious (adj) | rõ ràng |
| awareness (n) | nhận thức |
| energy (n) | năng lượng |

## 2. Grammar + answer choices vocabulary

### Conditions

| Từ / Cụm từ | Nghĩa |
|---|---|
| mayor (n) | thị trưởng |
| single-use plastic | nhựa dùng một lần |
| renewable energy | năng lượng tái tạo |
| environment (n) | môi trường |

### Time clauses

| Cụm từ | Nghĩa |
|---|---|
| get back | quay về |

## 3. Environment vocabulary

| Từ / Cụm từ | Nghĩa |
|---|---|
| pesticide (n) | thuốc trừ sâu |
| waste (n,v) | rác; lãng phí |
| chemical fertilizer | phân bón hóa học |
| global warming | nóng lên toàn cầu |
| climate change | biến đổi khí hậu |

### Phrasal verbs / options

| Cụm từ | Nghĩa |
|---|---|
| go down | giảm xuống |
| end up | cuối cùng rơi vào / kết thúc ở |
| increase (v) | tăng |

## 4. Plastic pollution

| Từ / Cụm từ | Nghĩa |
|---|---|
| microplastics (n) | vi nhựa |
| dump (v) | đổ bỏ |
| get rid of | loại bỏ |
| release (v) | thải ra |
| leak (v) | rò rỉ |

## 5. Pollution

| Cụm từ | Nghĩa |
|---|---|
| traffic fumes | khí thải giao thông |
| air pollution | ô nhiễm không khí |

### Word family

| Từ | Nghĩa |
|---|---|
| pollute (v) | gây ô nhiễm |
| polluter (n) | chất / nguồn gây ô nhiễm |
| pollutant (n) | chất gây ô nhiễm |
| pollution (n) | sự ô nhiễm |

## 6. Ozone / environment

| Từ / Cụm từ | Nghĩa |
|---|---|
| ozone layer | tầng ozone |
| CFC (chlorofluorocarbon) | khí CFC |

### Similar verbs

| Từ | Nghĩa |
|---|---|
| injure | làm bị thương |
| hurt | làm đau |
| erode | bào mòn |

### Collocations

| Cụm từ | Nghĩa |
|---|---|
| damage the ozone layer | phá hủy tầng ozone |
| lead to | dẫn đến |

## 7. Synonyms / Opposites

| Từ / Cụm từ | Nghĩa |
|---|---|
| impact | tác động |
| affect (v) | ảnh hưởng |
| effect (n) | hiệu ứng / kết quả |
| consequence (n) | hậu quả |
| result (n) | kết quả |
| reuse | tái sử dụng |
| use something again | dùng lại |
| discourage | ngăn cản |
| permit | cho phép |
| promote | thúc đẩy |
| prohibit | cấm |
| save electricity / water | tiết kiệm điện / nước |
| waste (v) | lãng phí |

## 8. Reading vocabulary

| Từ / Cụm từ | Nghĩa |
|---|---|
| owner | chủ sở hữu |
| manager | quản lý |
| reduce the amount of | giảm lượng |
| trash | rác |
| go to waste | bị lãng phí |
| solution to | giải pháp cho |
| attempt (n) | nỗ lực |

## 9. Word formation

| Từ | Nghĩa |
|---|---|
| deforestation | nạn phá rừng |
| solution | giải pháp |

---

# UNIT 9 – TRAVEL & TOURISM

## 1. Vocabulary

| Từ | Nghĩa |
|---|---|
| delayed | bị hoãn |
| missed | bị lỡ |
| watched | xem |
| booked | đặt trước |
| alarm | chuông báo động |
| suitcase | vali |
| breakfast | bữa sáng |
| island | hòn đảo |
| message | tin nhắn |
| bedroom | phòng ngủ |
| resort | khu nghỉ dưỡng |
| wallet | ví |

## 2. Travel vocabulary

| Từ / Cụm từ | Nghĩa |
|---|---|
| magical city | thành phố kỳ diệu |
| beautiful buildings | tòa nhà đẹp |
| natural environment | môi trường tự nhiên |
| full of | đầy |
| attractions | điểm tham quan |
| Big Ben | Big Ben |
| Tower of London | Tháp London |
| National Park | công viên quốc gia |
| hiking | đi bộ đường dài |
| camping | cắm trại |

## 3. Food / travel

| Từ / Cụm từ | Nghĩa |
|---|---|
| seafood | hải sản |
| food poisoning | ngộ độc thực phẩm |
| accommodation | nơi ở / lưu trú |
| journey | hành trình |
| view | cảnh quan |
| site | địa điểm |

## 4. Tourism effects

| Từ / Cụm từ | Nghĩa |
|---|---|
| negative effect on | ảnh hưởng tiêu cực lên |
| take off | cất cánh |
| land | hạ cánh |
| delay | trì hoãn |
| miss a flight | lỡ chuyến bay |

## 5. Safety / travel

| Từ / Cụm từ | Nghĩa |
|---|---|
| fire alarm | chuông báo cháy |
| breathtaking views | quang cảnh ngoạn mục |
| vision | thị lực / tầm nhìn |
| image | hình ảnh |
| sight | cảnh vật |

## 6. Activities

| Cấu trúc / Cụm từ | Ghi nhớ |
|---|---|
| go skiing | đi trượt tuyết |
| used to + V | từng làm gì |
| used to V-ing | sai |

## 7. Synonyms / Opposites

| Từ / Cụm từ | Nghĩa |
|---|---|
| sustainable materials | vật liệu bền vững |
| native | bản địa |
| foreign | nước ngoài |
| ingredients | nguyên liệu |
| active | năng động |
| passive | thụ động |

## 8. Word formation

| Cụm từ | Nghĩa |
|---|---|
| solar power | năng lượng mặt trời |
| tourist attractions | điểm du lịch |

---

# UNIT 10 – TECHNOLOGY & EDUCATION

## 1. Vocabulary

| Từ | Nghĩa |
|---|---|
| future | tương lai |
| computer | máy tính |
| function | chức năng |
| student | học sinh |
| education | giáo dục |
| communication | giao tiếp |
| prediction | dự đoán |
| question | câu hỏi |

## 2. Learning vocabulary

| Từ / Cụm từ | Nghĩa |
|---|---|
| pressed for time | thiếu thời gian |
| presentation | bài thuyết trình |
| online course | khóa học online |
| download | tải xuống |
| pronunciation | phát âm |
| robot teacher | giáo viên robot |

## 3. Education technology

| Từ / Cụm từ | Nghĩa |
|---|---|
| virtual classrooms | lớp học ảo |
| actual | thực tế |
| interactive | tương tác |
| private | riêng tư |
| artificial intelligence (AI) | trí tuệ nhân tạo |
| virtual reality | thực tế ảo |
| science fiction | khoa học viễn tưởng |
| high-quality classes | lớp học chất lượng cao |
| flexible classes | lớp học linh hoạt |

## 4. Skills

| Cụm từ | Nghĩa |
|---|---|
| problem-solving skills | kỹ năng giải quyết vấn đề |
| communication skills | kỹ năng giao tiếp |
| set up a reminder | đặt lời nhắc |
| get up | thức dậy |
| put up | dựng lên |

## 5. Technology words

| Từ / Cụm từ | Nghĩa |
|---|---|
| native speakers | người bản xứ |
| forum | diễn đàn |
| blog | blog |
| hardware | phần cứng |
| software | phần mềm |
| e-learning | học trực tuyến |
| numerous advantages | nhiều lợi ích |

## 6. Opposites

| Từ / Cụm từ | Nghĩa |
|---|---|
| complex issues | vấn đề phức tạp |
| complicated | phức tạp |
| simple | đơn giản |
| reasonable price | giá hợp lý |
| high price | giá cao |

## 7. Word formation

| Từ / Cụm từ | Nghĩa |
|---|---|
| educational apps | ứng dụng giáo dục |
| creativity | sự sáng tạo |

---

# FINAL TEST – MIXED REVIEW

## Environment + Travel + Tech

| Cụm từ | Nghĩa |
|---|---|
| charity event | sự kiện từ thiện |
| raise money | gây quỹ |
| neighborhood | khu dân cư |
| connected to the Internet | kết nối Internet |
| solve a problem | giải quyết vấn đề |
| cheap flights | vé máy bay rẻ |
| non-renewable energy | năng lượng không tái tạo |
| sign up | đăng ký |
| attend online classes | tham gia lớp online |
| World Heritage Site | di sản thế giới |

## Travel problems

| Cụm từ | Nghĩa |
|---|---|
| break down | hỏng xe / máy |
| stop working | ngừng hoạt động |

## Tourism / environment

| Cụm từ | Nghĩa |
|---|---|
| tourist destination | điểm đến du lịch |
| portable TV | TV di động |
| eco-tourism | du lịch sinh thái |
| environmental awareness | nhận thức môi trường |

## Reading extra vocabulary

| Từ / Cụm từ | Nghĩa |
|---|---|
| vacation | kỳ nghỉ |
| distance | khoảng cách |
| rainforest | rừng mưa nhiệt đới |
| pollution levels | mức độ ô nhiễm |
| private transport | phương tiện cá nhân |
| developing areas | khu vực đang phát triển |
| deforestation | phá rừng |
| intervention | sự can thiệp |
| unfixable | không thể sửa chữa |

## E-learning reading

| Cụm từ | Nghĩa |
|---|---|
| advanced technology | công nghệ tiên tiến |
| instant access | truy cập tức thì |
| resources | tài nguyên |
| opportunity | cơ hội |
| flexible education | giáo dục linh hoạt |
| infrastructure | cơ sở hạ tầng |
| accessible | dễ tiếp cận |

---

# CỤM RẤT QUAN TRỌNG NÊN HỌC THUỘC

## Environment

| Cụm từ | Nghĩa |
|---|---|
| single-use plastic | nhựa dùng một lần |
| renewable energy | năng lượng tái tạo |
| global warming | nóng lên toàn cầu |
| climate change | biến đổi khí hậu |
| air pollution | ô nhiễm không khí |
| ozone layer | tầng ozone |
| reduce pollution | giảm ô nhiễm |
| protect the environment | bảo vệ môi trường |
| raise awareness | nâng cao nhận thức |
| take action | hành động |

## Travel

| Cụm từ | Nghĩa |
|---|---|
| tourist attraction | điểm du lịch |
| tourist destination | điểm đến |
| food poisoning | ngộ độc thực phẩm |
| accommodation | chỗ ở |
| delayed flight | chuyến bay bị hoãn |
| breathtaking views | phong cảnh ngoạn mục |

## Education / Technology

| Cụm từ | Nghĩa |
|---|---|
| online course | khóa học trực tuyến |
| virtual classroom | lớp học ảo |
| artificial intelligence | trí tuệ nhân tạo |
| communication skills | kỹ năng giao tiếp |
| problem-solving skills | kỹ năng giải quyết vấn đề |
| sign up for | đăng ký |
| set up a reminder | đặt lời nhắc |

---

# Ghi nhớ nhanh trước khi kiểm tra

## Dễ ra chọn đáp án

- **damage** thường đi với: `the environment`, `the ozone layer`.
- **lead to** = dẫn đến.
- **solution to** = giải pháp cho.
- **negative effect on** = ảnh hưởng tiêu cực lên.
- **miss a flight** = lỡ chuyến bay.
- **delayed flight** = chuyến bay bị hoãn.
- **used to + V** là đúng.
- **used to V-ing** là sai trong nghĩa “đã từng”.

## Cặp dễ nhầm

| Từ | Loại từ / Nghĩa |
|---|---|
| affect | động từ: ảnh hưởng |
| effect | danh từ: ảnh hưởng / kết quả |
| permit | cho phép |
| prohibit / ban | cấm |
| promote | thúc đẩy |
| discourage | ngăn cản |
| native | bản địa |
| foreign | nước ngoài |
| simple | đơn giản |
| complicated / complex | phức tạp |
